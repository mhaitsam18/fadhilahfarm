		...
		<div class="col-xs-4">
			...
			?>
				...
				<div class="row">
					...
					echo form_open(base_url(akses() . '/transaksi/orderan/approve'), array('class' => 'col-md-3 text-right'));
					?>
					<input type="hidden" name="penjualanid" value="<?= $row->penjualan_id; ?>" />
					<input type="hidden" name="invoice" value="<?= $row->invoice; ?>" />
					<input type="hidden" name="status" value="lunas" />
					<button type="submit" onclick="return confirm('Yakin telah menerima dana transfer ini?');" class="btn btn-primary btn-flat">Terima</button>
					<?php
					echo form_close();
					?>
					<?php
					echo validation_errors();
					echo form_open(base_url(akses() . '/transaksi/orderan/approve'), array('class' => 'col-md-9 text-left'));
					?>
					<input type="hidden" name="penjualanid" value="<?= $row->penjualan_id; ?>" />
					<input type="hidden" name="invoice" value="<?= $row->invoice; ?>" />
					<input type="hidden" name="status" value="draft" />
					<button type="submit" onclick="return confirm('Yakin telah menolak bukti transfer ini?');" class="btn btn-danger btn-flat">Tolak</button>
					<?php
					echo form_close();
					?>
				<?php
